# ------------------------------------------------------------------------------
# Design Configuration for PULPino Synthesis (called from synt.tcl)
# ------------------------------------------------------------------------------
...
set_attribute init_hdl_search_path "$RTL $RTL/includes $COMPONENTS \
  $IPS/adv_dbg_if/rtl \
  $IPS/apb/apb_event_unit \
  ...
  $IPS/apb/apb_node \
  $IPS/apb/apb_pulpino \
  $IPS/apb/apb_spi_master \
  $IPS/apb/apb_timer \
  $IPS/apb/apb_uart \
  $IPS/apb/apb_uart_sv \
  $IPS/apb/apb2per \
  $IPS/axi/axi_mem_if_DP \
  $IPS/axi/axi_node \
  $IPS/axi/axi_slice \
  $IPS/axi/axi_slice_dc \
  $IPS/axi/axi_spi_master \
  $IPS/axi/axi_spi_slave \
  $IPS/axi/axi2apb \
  $IPS/axi/core2axi \
  $IPS/riscv \
  $IPS/riscv/include"
...
# -------------------------------------------------------
# Technology library paths (RAM, pads, clock cells, etc.)
# -------------------------------------------------------

# WARNING: for genus legacy this exact syntax is needed with \ in the ends
...
# -------------------------------------------------------
# Liberty (.lib) files for synthesis (looks like its corner conditions from the above libraries)
# -------------------------------------------------------
...
# ----------------------------------------------------------------------------
# RTL File Lists (Split by language)
# ----------------------------------------------------------------------------


# WARNING: do not insert comment between " "
...
set SV_FILES "
	...
	core_region.sv peripherals.sv pulpino_top.sv pulpino_top_pads.sv \
"
